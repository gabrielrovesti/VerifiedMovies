...
export default async function CreateIssuers(){
    ...
    const result_1 = await contract.methods.createChildTrustedDid(childAddress_1, signature).send({from: accounts[2]});

    console.log("Issuer 2 Created", result_1);

    const childAddress_2 = "0x90f79bf6eb2c4f870365e785982e1f101e93b906"; //account[3]

    const result_2 = await contract.methods.createChildTrustedDid(childAddress_2, signature).send({from: accounts[3]});

    console.log("Issuer 3 Created", result_2);

    const childAddress_3 = "0x15d34aaf54267db7d7c367839aaf71a00a2c6a65"; //account[4]

    const result_3 = await contract.methods.createChildTrustedDid(childAddress_3, signature).send({from: accounts[4]});

    console.log("Issuer 4 Created", result_3);
}